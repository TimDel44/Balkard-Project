...
carteArmure::carteArmure(string nom, string nomStat, string niveau, int a, int s, int c) :carte(nom), nomStat(nomStat), niveau(niveau), alteration(a), statistique(s), cost(c) {}
...
void carteArmure::sfafficher(sf::RenderWindow* window, int xpos, int deck/*, int ypos*/)
{
	...
	niveau.setPosition(preNiveau.getPosition().x + 70.f, preNiveau.getPosition().y);
